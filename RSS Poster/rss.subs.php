...
{
	global $txt, $context, $tag_attrs, $feedcount, $maxitemcount, $insideitem, $tag, $modSettings;
					...
					else
					{
						...
						for ($i = 0; $i <= ($feedcount); $i++)
						{
							...
							if ($db->affected_rows() == 0)
							{
								...
								// Create the Post
								$msg_title = Util::htmlspecialchars(($feed['html'] ? $context['feeditems'][$i]['title'] : strip_tags($context['feeditems'][$i]['title'])), ENT_QUOTES);
								$msg_title = trim($msg_title);
								$msg_body = '';
								
								if ($feed['html'])
								{
									$msg_body =  Util::htmlspecialchars($context['feeditems'][$i]['description'], ENT_QUOTES);
								...
								{ 
									...
									$msg_body =  Util::htmlspecialchars( strip_tags($context['feeditems'][$i]['description']), ENT_QUOTES);
            ...
            foreach ( $json_feed_object->entries as $entry )
            {
               ...
               // echo "<small>{$published}</small>";
                //echo "<p>{$entry->content}</p>";
                 ...
                 $context['feeditems'][$feedcount]['description'] = $context['feeditems'][$feedcount]['description'];
